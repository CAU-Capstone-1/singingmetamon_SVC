...
def preprocess_for_training(train_A_dir, train_B_dir, cache_folder):
    ...
    wavs_A = preprocess.get_wave_files(wav_dir=train_A_dir, sr=sampling_rate)
    wavs_B = preprocess.get_wave_files(wav_dir=train_B_dir, sr=sampling_rate)
    ...
    log_f0s_mean_A, log_f0s_std_A = preprocess.logf0_info(f0s=f0s_A)
    log_f0s_mean_B, log_f0s_std_B = preprocess.logf0_info(f0s=f0s_B)
    ...
    coded_sps_A_transposed = preprocess.transpose_list(lst=coded_sps_A)
    coded_sps_B_transposed = preprocess.transpose_list(lst=coded_sps_B)
    ...
    print("Preprocessing time : {:.4f}seconds.".format(
        end_time - start_time))
...
if __name__ == '__main__':
    ...
    train_A_dir_default = './data/default/A'
    train_B_dir_default = './data/default/B'
